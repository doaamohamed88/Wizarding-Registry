import { r as __toESM, t as require_react } from "./react-fj7Lnh_h.js";
...
function convertRoutesToDataRoutes(routes, mapRouteProperties$1, parentPath, manifest) {
	...
	return routes.map((route, index) => {
		...
		if (isIndexRoute(route)) {
			let indexRoute = _extends$1({}, route, mapRouteProperties$1(route), { id });
			manifest[id] = indexRoute;
			return indexRoute;
		} else {
			let pathOrLayoutRoute = _extends$1({}, route, mapRouteProperties$1(route), {
				id,
				children: void 0
			});
...
var json = function json$1(data, init) {
	...
	return new Response(JSON.stringify(data), _extends$1({}, responseInit, { headers }));
};
...
var redirect = function redirect$1(url, init) {
	...
	return new Response(null, _extends$1({}, responseInit, { headers }));
};
/**
...
function createRouter(init) {
	...
	async function startNavigation(historyAction, location, opts) {
		...
		pendingNavigationController = null;
		completeNavigation(location, _extends$1({ matches: updatedMatches || matches }, getActionDataForCommit(pendingActionResult), {
			loaderData,
			errors
		}));
	}
	async function handleAction(request, location, submission, matches, isFogOfWar, opts) {
		...
		if (isRedirectResult(result)) {
			...
			else replace$1 = normalizeRedirectLocation(result.response.headers.get("Location"), new URL(request.url), basename, init.history) === state.location.pathname + state.location.search;
	...
	async function handleLoaders(request, location, matches, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace$1, initialHydration, flushSync, pendingActionResult) {
		...
		if (isFogOfWar) {
			if (shouldUpdateNavigationState) {
				...
				updateState(_extends$1({ navigation: loadingNavigation }, actionData !== void 0 ? { actionData } : {}), { flushSync });
			}
			let discoverResult = await discoverRoutes(matches, location.pathname, request.signal);
			if (discoverResult.type === "aborted") return { shortCircuited: true };
			else if (discoverResult.type === "error") {
		...
		if (future.v7_partialHydration && initialHydration && state.errors) errors = _extends$1({}, state.errors, errors);
	...
	*/
	async function startRedirectNavigation(request, redirect$1, isNavigation, _temp2) {
		...
		if (redirectPreserveMethodStatusCodes.has(redirect$1.response.status) && activeSubmission && isMutationMethod(activeSubmission.formMethod)) await startNavigation(redirectHistoryAction, redirectLocation, {
			submission: _extends$1({}, activeSubmission, { formAction: location }),
...
function getMatchesToLoad(history, state, matches, submission, location, initialHydration, skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult) {
	...
	fetchLoadMatches.forEach((f, key) => {
		...
		else shouldRevalidate = shouldRevalidateLoader(fetcherMatch, _extends$1({
			currentUrl,
...
async function loadLazyRouteModule(route, mapRouteProperties$1, manifest) {
	...
	Object.assign(routeToUpdate, _extends$1({}, mapRouteProperties$1(routeToUpdate), { lazy: void 0 }));
}
...
function processLoaderData(state, matches, results, pendingActionResult, revalidatingFetchers, fetcherResults, activeDeferreds) {
	...
	revalidatingFetchers.forEach((rf) => {
		...
		if (controller && controller.signal.aborted) return;
			...
			if (!(errors && errors[boundaryMatch.route.id])) errors = _extends$1({}, errors, { [boundaryMatch.route.id]: result.error });
			state.fetchers.delete(key);
...
function stripHashFromPath(path) {
	...
	return createPath(_extends$1({}, parsedPath, { hash: "" }));
}
...
var DataRouterHook = /* @__PURE__ */ function(DataRouterHook$1) {
	DataRouterHook$1["UseBlocker"] = "useBlocker";
	DataRouterHook$1["UseRevalidator"] = "useRevalidator";
	DataRouterHook$1["UseNavigateStable"] = "useNavigate";
	return DataRouterHook$1;
}(DataRouterHook || {});
var DataRouterStateHook = /* @__PURE__ */ function(DataRouterStateHook$1) {
	DataRouterStateHook$1["UseBlocker"] = "useBlocker";
	DataRouterStateHook$1["UseLoaderData"] = "useLoaderData";
	DataRouterStateHook$1["UseActionData"] = "useActionData";
	DataRouterStateHook$1["UseRouteError"] = "useRouteError";
	DataRouterStateHook$1["UseNavigation"] = "useNavigation";
	DataRouterStateHook$1["UseRouteLoaderData"] = "useRouteLoaderData";
	DataRouterStateHook$1["UseMatches"] = "useMatches";
	DataRouterStateHook$1["UseRevalidator"] = "useRevalidator";
	DataRouterStateHook$1["UseNavigateStable"] = "useNavigate";
	DataRouterStateHook$1["UseRouteId"] = "useRouteId";
	return DataRouterStateHook$1;
}(DataRouterStateHook || {});
...
function useBlocker(shouldBlock) {
	...
	let blockerFunction = import_react.useCallback((arg) => {
		...
		return shouldBlock({
			currentLocation: _extends({}, currentLocation, { pathname: stripBasename(currentLocation.pathname, basename) || currentLocation.pathname }),
			nextLocation: _extends({}, nextLocation, { pathname: stripBasename(nextLocation.pathname, basename) || nextLocation.pathname }),
			historyAction
		});
...
function RouterProvider(_ref) {
	...
	import_react.useEffect(() => logV6DeprecationWarnings(future, router.future), [router, future]);
	return /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, /* @__PURE__ */ import_react.createElement(DataRouterContext.Provider, { value: dataRouterContext }, /* @__PURE__ */ import_react.createElement(DataRouterStateContext.Provider, { value: state }, /* @__PURE__ */ import_react.createElement(Router, {
		basename,
		location: state.location,
...
//#endregion
export { isRouteErrorResponse as $, useNavigation as A, useRoutes as B, useHref as C, useMatch as D, useLocation as E, useResolvedPath as F, IDLE_FETCHER as G, AbortedDeferredError as H, useRevalidator as I, createPath as J, createBrowserHistory as K, useRouteError as L, useOutlet as M, useOutletContext as N, useMatches as O, useParams as P, invariant as Q, useRouteId as R, useBlocker as S, useLoaderData as T, Action as U, useRoutesImpl as V, ErrorResponseImpl as W, defer as X, createRouter as Y, generatePath as Z, mapRouteProperties as _, MemoryRouter as a, redirect as at, useAsyncError as b, Outlet as c, resolvePath as ct, Router as d, joinPaths as et, RouterProvider as f, logV6DeprecationWarnings as g, createRoutesFromChildren as h, LocationContext as i, parsePath as it, useNavigationType as j, useNavigate as k, Route as l, stripBasename as lt, createMemoryRouter as m, DataRouterContext as n, matchPath as nt, Navigate as o, redirectDocument as ot, Routes as p, createHashHistory as q, DataRouterStateContext as r, matchRoutes as rt, NavigationContext as s, replace as st, Await as t, json as tt, RouteContext as u, warning as ut, renderMatches as v, useInRouterContext as w, useAsyncValue as x, useActionData as y, useRouteLoaderData as z };
//# sourceMappingURL=dist-CcJszOye.js.map